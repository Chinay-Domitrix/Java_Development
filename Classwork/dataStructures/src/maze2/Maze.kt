...
class Maze : JPanel(), KeyListener, MouseListener {
	...
	var shrink = 15
	var location = Location(0, 2)
	var explorer = Explorer(location, 0, 13, RED)
	fun setBoard() {
		try {
			/*BufferedReader(FileReader(File("Classwork/dataStructures/src/maze/Maze.txt"))).use {
				var txt: String
				while (it.readLine().also { s -> txt = s } != null) {
					list.add(txt)
	...
	public override fun paintComponent(g: Graphics) {
		...
		if (!draw3D) {
			for (i in list.indices) for (j in 0 until list[i].length) {
				list[i].split("".toRegex())/*.toTypedArray()*/.run {
					if (this[j] == "#") {
