...
public class BlockBreak2 extends JPanel implements KeyListener, Runnable {
	...
	private BlockBreak2() {
		...
		f = new Font("Comic Sans MS", BOLD, 32);  // Set Font  https://docs.oracle.com/javase/7/docs/api/java/awt/Font.html
		frame.add(this);
		frame.setSize(frameWidth, frameHeight);
		frame.setVisible(true);
		frame.addKeyListener(this); // Needed to use KeyListener
		frame.setDefaultCloseOperation(EXIT_ON_CLOSE);
//		Create a pattern of WiggleBlocks (you could do the same with regular static blocks)
	...
	public void paintComponent(Graphics g) { // Need to have this method to complete the painting
		...
		g2d.setColor(RED);
		// Get the Blocks (or Wiggle Blocks) and paint one at a time.   Paint the goal pink and everything else red
		blocks.forEach(b -> {
			g2d.setColor(RED);
			if (b.isGoal()) g2d.setColor(PINK);
			g2d.fill(b.getR());
		});
		g2d.setColor(BLUE);
		g2d.setFont(f);  // NOTE: Font initialized in the constructor
		g2d.drawString(msg, 100, 30);  // Write text from msg String
		g2d.setColor(new Color(0, 255, 130));  // Another way to set color.   Parameters are red, green, blu on a scale of 0-255
	...
	public void run() {
		...
		while (!win) { // Keep going until win or window is closed
			...
			if (blockInteraction(ball1)) {
				win = true;
				msg = "YOU WIN!";
			} else msg = (blocks.size() > 0) ? (blocks.size() + " BLOCKS") : "NO MORE BLOCKS!";
			try {
	...
	// This method was the previous checkcollision method, but now the check if you ran into the
	...
	public void keyPressed(KeyEvent ke) {
		...
		else if (ke.getKeyCode() == 37) left = true;
		else if (ke.getKeyCode() == 38) up = true;
		else if (ke.getKeyCode() == 40) down = true;
	}

	public void keyReleased(KeyEvent ke) {
		requireNonNull(ke);
		if (ke.getKeyCode() == 39) right = false;
		else if (ke.getKeyCode() == 37) left = false;
		else if (ke.getKeyCode() == 38) up = false;
		else if (ke.getKeyCode() == 40) down = false;
	}
