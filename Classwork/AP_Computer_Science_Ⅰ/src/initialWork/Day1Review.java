	...
	public static void main(String[] args) {
		try (Scanner scanner = new Scanner(in)) {
			...
			switch (scanner.nextInt()) {
				...
				case 2 -> rangeClosed(1, 6).forEachOrdered(i -> {
					iterate(6, j -> j > i, j -> j - 1).mapToObj(j -> "*").forEachOrdered(out::print);
					out.println();
				});
				case 3 -> range(1, 5).forEachOrdered(i -> {
					iterate(5, j -> j > i, j -> j - 1).forEachOrdered(out::print);
					if (i % 2 == 1)
						out.print('*');
					out.println();
				});
				case 4 -> range(0, 15).map(i -> (int) (random() * 15) + 1).forEachOrdered(rand -> {
					range(0, rand).map(j -> '*').forEachOrdered(out::print);
					out.println();
				});
				case 5 -> range(0, 5).forEachOrdered(i -> {
