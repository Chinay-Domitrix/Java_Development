...
class NameAndDescriptor {
	private NameAndDescriptor() {
		final var NamesAndDescriptors = new String[][]{new String[]{"Pranav", "Sashi", "Nick", "Arsh", "Kush", "Veer", "Dylan", "Yathin", "Omkar", "Tridib", "Joe", "Aditya", "Ishan", "Jill", "Abhinaya", "Chirag", "Harshitha", "Eshita", "Jeslin", "Amit", "Sridula"}, new String[]{"Confused", "Memorable", "Robotics", "Interesting", "Procrastinator", "Happy", "Crazy", "Table Tennis", "Basketball", "Hardworking", "Zombie", "Video Games", "Smart", "Athletic", "Outgoing", "Curious", "Friendly", "Creative", "Badminton", "Artistic"}};
	...
	private static int logic(@NotNull final String[]... a) throws IllegalStateException {
		...
		do {
			...
			if (choice.equalsIgnoreCase("quit")) return score;
				...
				if (redo.equalsIgnoreCase("no")) return score;
				else if (redo.equalsIgnoreCase("yes")) out.println();
				else throw new IllegalStateException("Unexpected value: " + redo);
			} else if (!choice.equalsIgnoreCase(answer)) {
				if (score != 0) score--;
	...
	private static String pickName(@NotNull final String[]... a) {
		...
		if (name.equals("Pranav")) name += format(" %s", c[(int) (random() * c.length - 1)]);
		return name;
	}
}
