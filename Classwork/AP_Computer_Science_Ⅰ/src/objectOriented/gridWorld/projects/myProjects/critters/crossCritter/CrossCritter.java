...
//		Adds every Actor in the same column as the CrossCritter to the ArrayList removable
		for (int i = 0; i < getGrid().getNumRows(); i++)
			removable.add(getGrid().get(new Location(i, getLocation().getCol())));
//		Adds every Actor in the same row as the CrossCritter to the ArrayList removable
