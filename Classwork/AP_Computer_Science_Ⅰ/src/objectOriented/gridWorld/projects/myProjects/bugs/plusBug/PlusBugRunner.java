...
public class PlusBugRunner {
	private PlusBugRunner() {
		...
		var plusBugs = new PlusBug[]{new PlusBug(1, locations[0]), new PlusBug(2, locations[1]), new PlusBug(3, locations[2])};
		for (int i = 0; i < plusBugs.length; i++) world.add(locations[i], plusBugs[i]);
		world.show();
	}
