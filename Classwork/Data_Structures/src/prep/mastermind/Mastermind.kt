...
fun main() = try {
	Scanner(File("Classwork/dataStructures/src/prep/mastermind/Mastermind.txt")).use { fileReader ->
		...
		for (i in 1 until dataSet.size) {
			...
			failT1.forEach {
				key2.add((key[0] as CharArray)[it])
				current2.add(current[it])
			}
			current2.forEach {
				if (key2.contains(it)) {
					matchT2++
