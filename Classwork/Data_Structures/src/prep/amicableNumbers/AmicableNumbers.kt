...
fun main() = try {
	Scanner(File("Classwork/dataStructures/src/prep/amicableNumbers/Amicable_Numbers.txt")).use { fileReader ->
...
private fun formatFactorString(input: IntArray): String {
	...
	factorStringBuilder = StringBuilder(factorStringBuilder.toString().trim { it <= ' ' }).deleteCharAt(
		factorStringBuilder.lastIndexOf(",")
	)
