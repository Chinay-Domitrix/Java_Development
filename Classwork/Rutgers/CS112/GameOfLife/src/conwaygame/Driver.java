...
public class Driver {
	...
	public static final String[] constructorNames = {"Create Default Game", "Create Game with Input File", "Create Custom Game"};
	public static final String[] inputNames = {"Open"};
	public static final String[] createNames = {"-", "+", "-", "+", "Save and Create"};
	public static final String[] methodNames = {"Cell State", "Is Alive", "Alive Neighbors", "Next Generation", "Next N Generations", "Communities", "Reset", "Save Grid"};
	public static final Button[] OPTIONS = {new Button(6, 95, 4, 3, "Back", true), new Button(16, 95, 4, 3, "Quit", true)};
	...
	public static void run(Button btn) {
		if (inArray(btn.name, optionNames)) {
				...
				case "Save and Create":
					...
					try {
						...
						if (newInputFile.createNewFile()) {
							...
							inputWriter.write(activeBoard.cols + "\n"); // cols
							inputWriter.write(activeBoard.rows + "\n"); // then rows
							int aliveCells = 0;
							...
							inputWriter.write(aliveCells + "\n");

							for (boolean[] row : activeBoard.board) {
								for (boolean cell : row) {
									String spacing = (cell) ? "   " : "  ";
									inputWriter.write(cell + spacing);
								}
								inputWriter.write("\n");
							}
							inputWriter.close();
			...
			for (int i = 0; i < Math.min(r, activeBoard.rows); i++) {
				if (Math.min(c, activeBoard.cols) >= 0)
					System.arraycopy(activeBoard.board[i], 0, newCells[i], 0, Math.min(c, activeBoard.cols));
			}
				...
				case "Next N Generations":
					...
					while (true) {
						if (StdDraw.hasNextKeyTyped()) {
							int keystroke = StdDraw.nextKeyTyped();
