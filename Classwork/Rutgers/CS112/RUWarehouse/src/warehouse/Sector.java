...
public class Sector {
	...
	public void sink(int index) {
		while (index * 2 <= currentSize) {
			int smallestChild = ((((index * 2) + 1) > currentSize) || (products[index * 2].getPopularity() < products[index * 2 + 1].getPopularity())) ? (index * 2) : ((index * 2) + 1);
    ...
    public String toString() {
        ...
        sectorString.append("null");
        for (int i = 1; i <= currentSize; i++) sectorString.append("; ").append(products[i].toString());
        return sectorString + "}";
    }
}
