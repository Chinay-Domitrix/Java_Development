...
import static kindergarten.StdOut.print;
...
public final class StdIn {
	...
	public static String readAll() {
		...
		var result = scanner.useDelimiter(EVERYTHING_PATTERN).next();
	...
	public static int readInt() {
		try {
			...
			throw new NoSuchElementException("attempts to read an 'int' value from standard input, but no more tokens are available");
		}
	 ...
	 */
	public static String @NotNull [] readAllStrings() {
		// we could use readAll.trim().split(), but that's not consistent
		// because trim() uses characters 0x00..0x20 as whitespace
		var tokens = WHITESPACE_PATTERN.split(readAll());
		if ((tokens.length == 0) || (tokens[0].length() > 0)) return tokens;
		// don't include first token if it is leading whitespace
		return range(0, tokens.length - 1).mapToObj(i -> tokens[i + 1]).toArray(String[]::new);
	}
	 ...
	 */
	public static String @NotNull [] readAllLines() {
		return iterate(0, i -> hasNextLine(), i -> i + 1).mapToObj(i -> readLine()).toArray(String[]::new);
	}
	...
	public static int[] readAllInts() {
		return stream(readAllStrings()).mapToInt(Integer::parseInt).toArray();
	}
	...
	public static long[] readAllLongs() {
		return stream(readAllStrings()).mapToLong(Long::parseLong).toArray();
	}
	...
	public static double[] readAllDoubles() {
		return stream(readAllStrings()).mapToDouble(Double::parseDouble).toArray();
	}
	...
	public static void resync() {
		setScanner(new Scanner(new BufferedInputStream(in), CHARSET_NAME));
	}
	...
	public static void setFile(String filename) {
		try {
			setScanner(new Scanner(new File(filename), CHARSET_NAME));
		} catch (FileNotFoundException ioe) {
			err.println("Could not open " + filename);
		}
	}
	...
	public static void main(String[] args) {
		...
		println("Your boolean was: " + b);
		println();
		print("Type a double: ");
		double c = readDouble();
		println("Your double was: " + c);
		println();
	}
