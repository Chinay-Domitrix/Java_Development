...
public final class p014 implements EulerSolution {
	...
	private int collatzChainLengthDirect(@NotNull BigInteger n) {
		...
		else if (!n.testBit(0)) return collatzChainLength(n.shiftRight(1)) + 1; // If n is even
		else return collatzChainLength(n.multiply(BigInteger.valueOf(3)).add(ONE)) + 1; // Else n is odd
	}
}