...
public final class LibraryTest {
	...
	public void testIsSquare() {
		...
		assertFalse(Library.isSquare(MIN_VALUE));
		assertFalse(Library.isSquare(MIN_VALUE + 1));
		...
		assertFalse(Library.isSquare(MAX_VALUE - 1));
		assertFalse(Library.isSquare(MAX_VALUE));
	}
	...
	public void testTotientRandomly() {
		...
		for (int i = 0; i < trials; i++) {
			...
			int tot = 0;
			for (int j = 1; j <= n; j++) if (Library.gcd(j, n) == 1) tot++;
