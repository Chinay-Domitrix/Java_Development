...
public final class p086 extends EulerSolution {
	...
	private final List<Set<List<Integer>>> solutions = new ArrayList<>();
	// cumulativeSolutions.get(m) = solutions.get(0).size() + solutions.get(1).size() + ... + solutions.get(m).size().
	private final List<Integer> cumulativeSolutions = new ArrayList<>();
	...
	private void findSplits(int a, int b, int c, int limit) {
		for (int x = 1; x < a; x++) {
			...
			if (Math.min(Math.min(
					...
					(y + b) * (y + b) + x * x),
					(b + x) * (b + x) + y * y) == c * c) {
				int max = Math.max(Math.max(x, y), b);
				if (max < limit) {
