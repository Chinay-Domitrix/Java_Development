...
public final class p208 extends EulerSolution {
	...
	String run() {
		...
		HashMap<State, BigInteger> reachable = new HashMap<>();
		...
		for (int i = 0; i < LIMIT; i++) {
			HashMap<State, BigInteger> newReachable = new HashMap<>();
			for (Entry<State, BigInteger> entry : reachable.entrySet()) {
				State state = entry.getKey();
				BigInteger ways = entry.getValue();
				State acw = state.move(true);
				BigInteger temp = newReachable.getOrDefault(acw, ZERO);
