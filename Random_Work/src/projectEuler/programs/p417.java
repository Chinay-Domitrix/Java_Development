...
public final class p417 extends EulerSolution {
	...
	private static int[] calcPrimePowerPeriods(@NotNull long[] primePowersAndTotients, int[] smallestPrimeFactor) {
		...
		for (int i = 0; i < primePowersAndTotients.length; i++) {
			...
			while (remainingFactors > 1) {
				...
				if (powMod(10, period / q, primePow) == 1) period /= q;
				remainingFactors /= q;
			}
			result[i] = period;
		}
	...
	private static long sumAllPeriods(int[] periods) throws AssertionError {
		...
		for (int i = 3; i <= LIMIT; i++) {
			int n = i;
			n >>>= numberOfTrailingZeros(n); // Remove all factors of 2
			while (n % 5 == 0) // Remove all factors of 5 n /= 5;
				assert (n <= 1) || (periods[n] != 0);
			sum += periods[n];
		}
		return sum;
	}
	...
	@NotNull String run() {
		...
		int[] smallestPrimeFactor = listSmallestPrimeFactors(LIMIT); // Requires 400 MB
