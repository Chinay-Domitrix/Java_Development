...
public final class p149 extends EulerSolution {
	...
	@NotNull String run() {
		...
		var rand = new LfgRandom();
		stream(grid).forEachOrdered(ints -> setAll(ints, x -> rand.next()));
	...
	private static final class LfgRandom {
		...
		int next() {
			...
			if (k <= 55) {
				...
				result = ((getHistory(24) + getHistory(55) + 1000000) % 1000000) - 500000; // Don't increment k, to prevent overflow
