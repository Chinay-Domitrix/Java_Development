...
public final class p216 extends EulerSolution {
	 ...
	 * https://code.google.com/archive/p/fun-math-problems/source/default/source?page=15 , programs.p216.tex
	 */
	...
	@NotNull String run() {
		...
		range(2, sequence.length).forEachOrdered(i -> sequence[i] = (2L * i * i) - 1);
		...
		for (int i = 2; i < sequence.length; i++) {
			...
			if (1 < term && term <= LIMIT * 2) {
				...
				iterate(i + p, j -> j < sequence.length, j -> j + p).forEachOrdered(j -> {
					assert sequence[j] % p == 0;
					do sequence[j] /= p;
					while (sequence[j] % p == 0);
				});
				// j starts at the smallest number such that j > i and j = -i mod p
				iterate(i + (((p - i) << 1) % p), j -> j < sequence.length, j -> j + p).forEachOrdered(j -> {
					assert sequence[j] % p == 0;
					do sequence[j] /= p;
					while (sequence[j] % p == 0);
				});
