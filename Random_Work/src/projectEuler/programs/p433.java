...
public final class p433 extends EulerSolution {
	...
	@NotNull String run() {
		rangeClosed(1, LIMIT).forEachOrdered(i -> exploreGcds(i, 0, 0));
		return Long.toString((sum << 1) + 12500002500000L);
	}
	...
	private void exploreGcds(int x, int y, int steps) {
		...
		sum += steps;
		iterate(y + (((y > 0) ? 1 : 2) * x), z -> z <= LIMIT, z -> z + x).forEachOrdered(z -> exploreGcds(z, x, steps + 1));
	}
}