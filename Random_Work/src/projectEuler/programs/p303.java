...
public final class p303 extends EulerSolution {
	...
	private static BigInteger findMinimumMultiple(int n) {
		...
		var feasible = new ArrayList<byte[]>();
		...
		for (int i = 0; feasible.get(i)[0] != 2; i++) { // Unbounded loop
			...
			byte[] cur = new byte[n];
			int digitMod = powMod(10, i, n);
			// Run time of O(n)
			IntStream.range(0, n).filter(j -> prev[j] > 0).forEachOrdered(j -> {
				cur[(j) % n] = prev[j];
				cur[(j + digitMod) % n] = 2;
		...
		for (int i = feasible.size() - 2; i >= 0; i--) {
			...
			for (int j = ((i == (feasible.size() - 2)) ? 1 : 0); j <= 2; j++) {
				...
				if (feasible.get(i)[newRem] > 0) {
					result = result.multiply(valueOf(10)).add(valueOf(j));
					remainder = newRem;
	...
	String run() {
		BigInteger sum = ZERO;
		for (int n = 1; n <= 10000; n++) sum = sum.add(findMinimumMultiple(n).divide(valueOf(n)));
		return sum.toString();
	}
}