...
import static projectEuler.programs.Fraction.ZERO;
import static projectEuler.programs.Library.binomial;

public final class p323 extends EulerSolution {
	...
	String run() {
		...
		for (int n = 1; n < expect.length; n++) {
			Fraction sum = ZERO;
			for (int k = 0; k < n; k++) {
				BigInteger binom = binomial(n, k);
				Fraction term = new Fraction(expect[k].numerator.multiply(binom), expect[k].denominator);
				sum = sum.add(term);
			}
			BigInteger twoPowN = ONE.shiftLeft(n);
			Fraction temp = sum.add(new Fraction(twoPowN));
			expect[n] = new Fraction(temp.numerator, temp.denominator.multiply(twoPowN.subtract(ONE)));
		}
		...
		Fraction scaled = new Fraction(ans.numerator.multiply(TEN.pow(DECIMALS)), ans.denominator);
		...
		assert frac.compareTo(new Fraction(ONE)) < 0;
		Fraction HALF = new Fraction(ONE, valueOf(2));
		if ((frac.compareTo(HALF) > 0) || (frac.equals(HALF) && whole.testBit(0))) whole = whole.add(ONE);
		StringBuilder temp = new StringBuilder(whole.toString());
		if (DECIMALS == 0) return whole.toString();
		while (temp.length() < DECIMALS + 1) temp.insert(0, "0");
