...
public final class p191 extends EulerSolution {
	...
	@NotNull String run() {
		...
		var numPrizeStrings = new long[NUM_DAYS + 1][MAX_ABSENT + 1][MAX_LATE + 1];
		...
		for (int i = 1; i <= NUM_DAYS; i++)
			for (int j = 0; j <= MAX_ABSENT; j++)
				for (int k = 0; k <= MAX_LATE; k++) {
					...
					if (j == 0) {
						sum = 0;
						for (int l = 0; l <= MAX_ABSENT; l++) sum += numPrizeStrings[i - 1][l][k]; // On time
						if (k > 0)
							for (int l = 0; l <= MAX_ABSENT; l++) sum += numPrizeStrings[i - 1][l][k - 1]; // Late
					} else sum = numPrizeStrings[i - 1][j - 1][k]; // Absent
