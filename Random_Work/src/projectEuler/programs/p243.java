...
public final class p243 extends EulerSolution {
	private static final Fraction TARGET = new Fraction(valueOf(15499), valueOf(94744));
	...
	String run() {
		...
		for (int p = 2; ; ) {
			totient = totient.multiply(valueOf(p - 1));
			denominator = denominator.multiply(valueOf(p));
			...
			if (new Fraction(totient, denominator).compareTo(TARGET) < 0) for (int i = 1; i < p; i++) {
				BigInteger numer = valueOf(i).multiply(totient);
				BigInteger denom = valueOf(i).multiply(denominator);
