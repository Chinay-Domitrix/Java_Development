...
public final class p431 extends EulerSolution {
	...
	private static double findRootSecant(double x0, double x1, double val) {
		...
		while (abs(x0 - x1) > 0.000000000001) {
			...
			y0 = y1;
			y1 = y2;
			if (abs(x0 - x1) < 0.0001) samples = max(1000000, samples);
			if (abs(x0 - x1) < 0.000001) samples = max(10000000, samples);
			if (abs(x0 - x1) < 0.00000001) samples = max(100000000, samples);
		}
		return x1;
	}
	...
	String run() {
		double sum = rangeClosed(20, 25).mapToDouble(i -> findRootSecant(0, RADIUS, i * i)).sum();
