...
public final class p265 extends EulerSolution {
	...
	private static boolean checkArrangement(int digits) {
		long temp = (digits & 4294967295L) | ((digits & 4294967295L) << TWO_POW_N); // Two copies
		int seen = range(0, TWO_POW_N).map(i -> 1 << ((int) (temp >>> i) & MASK)).reduce(0, (a, b) -> a | b);
		return seen == FULL_SET;
	}

	@NotNull String run() {
		long sum = iterate(67108865, i -> i < 134217728, i -> i + 2).filter(p265::checkArrangement).asLongStream().sum();
